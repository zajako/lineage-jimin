	...
	public static void onAction(L1PcInstance pc, L1Character cha) {
		...
		if (pc.getLawful() >= 0 && CharPosUtil.getZoneType(pc) != 1 
			...
			if (!attacker.isGmInvis()) {
				Broadcaster.broadcastPacket(attacker, new S_PinkName(attacker.getId(), 30));
			}
			attacker.getSkillEffectTimerSet().setSkillEffect(L1SkillId.STATUS_PINK_NAME, 30 * 1000);
		}
	}
}
