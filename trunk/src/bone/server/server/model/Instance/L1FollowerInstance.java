...
import bone.server.server.serverpackets.S_FollowerPack;
	...
	public void spawn(int npcId, int X, int Y, int H, short Map) {
		...
		if (l1npc != null) {
			...
			try {
				...
				Constructor<?> _constructor = Class.forName((new StringBuilder()).append("bone.server.server.model.Instance. ")
