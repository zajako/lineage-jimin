...
import bone.server.server.command.executor.L1UserCalc;
import bone.server.server.model.L1World;
import bone.server.server.model.Instance.L1PcInstance;
import bone.server.server.serverpackets.S_WhoAmount;
import bone.server.server.serverpackets.S_WhoCharinfo;
...
public class C_Who extends ClientBasePacket {
	...
	public C_Who(byte[] decrypt, LineageClient client) {
		...
		if (find != null) {
			...
			int r = ran.nextInt(2);
			int AddUser = (int)(L1World.getInstance().getAllPlayersToArray().length * 1.4) + r;
