...
import bone.server.Warehouse.ElfWarehouse;
import bone.server.Warehouse.PackageWarehouse;
import bone.server.Warehouse.PrivateWarehouse;
import bone.server.Warehouse.Warehouse;
...
import bone.server.server.model.shop.L1ShopBuyOrderList;
import bone.server.server.model.shop.L1ShopSellOrderList;
...
public class C_ShopAndWarehouse extends ClientBasePacket {
	...
	private void getItemToPackageWarehouse(L1PcInstance pc, int size) {
		...
		for (int i = 0; i < size; i++) {
			...
			bone.LogWareHouseAppend("패키지:", pc.getName(), "", item, count, objectId);
			}
	}
	...
	private void getItemToElfWarehouse(L1PcInstance pc, int size) {
		...
		for (int i = 0, objectId, count; i < size; i++) {
			...
			if (pc.getInventory().consumeItem(40494, 2)) {
				...
				bone.LogWareHouseAppend("요정:찾", pc.getName(), "", item, count, objectId);
			} else {
	...
	private void putItemToElfWarehouse(L1PcInstance pc, int size) {
		...
		for (int i = 0, objectId, count ; i < size ; i++) {
			...
			bone.LogWareHouseAppend("요정:맡", pc.getName(), "", item, count, objectId);			
		}
	}
	...
	private void getItemToClanWarehouse(L1PcInstance pc, int size) {
		...
		for (int i = 0, objectId, count; i < size; i++) {
			...
			bone.LogWareHouseAppend("혈맹:찾", pc.getName(), pc.getClanname(), item, count, objectId);
		}
		clanWarehouse.unlock(pc.getId());
	}
	...
	private void putItemToClanWarehouse(L1PcInstance pc, int size) {
		...
		for (int i = 0, objectId, count; i < size; i++) {
			...
			bone.LogWareHouseAppend("혈맹:맡", pc.getName(), pc.getClanname(), item, count, objectId);
		}
		clanWarehouse.unlock(pc.getId());
	}
	...
	private void getItemToPrivateWarehouse(L1PcInstance pc, int size) {
		...
		for (int i = 0, objectId, count ; i < size; i++) {
			...
			bone.LogWareHouseAppend("일반:찾", pc.getName(), "", item, count, objectId);
		}
	}
	...
	private void putItemToPrivateWarehouse(L1PcInstance pc, int size) {
		...
		for(int i = 0, objectId, count; i < size; i++) {
			...
			bone.LogWareHouseAppend("일반:맡", pc.getName(), "", item, count, objectId);
		}
	} 
	...
	private void writeLogbuyPrivateShop(L1PcInstance pc, L1PcInstance targetPc, L1ItemInstance item, int count, int price) {
		...
		bone.LogShopAppend("상점", pc.getName(), targetPc.getName(), item.getEnchantLevel(), itemadena, item.getBless(), count, item.getId());
	}
