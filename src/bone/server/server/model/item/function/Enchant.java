...
public class Enchant extends L1ItemInstance{
	...
	public void FailureEnchant(L1PcInstance pc, L1ItemInstance item) {
		...
		if (itemType == 1) { // 무기
			if (!item.isIdentified() || item.getEnchantLevel() == 0) {
				...
				.append(" ").append(nameId).toString();  // \f1%0%s 강렬하게%1 빛나더니 증발되어 사라집니다.
				sa = "$245";
			}
	...
	public void AttrEnchant(L1PcInstance pc, L1ItemInstance item, int item_id) {
		...
		} else if (item_id == L1ItemId.WATER_ENCHANT_WEAPON_SCROLL) { // 물의 무기 강화 주문서
		...
		} else if (item_id == L1ItemId.WIND_ENCHANT_WEAPON_SCROLL) { // 바람의 무기 강화 주문서
		...
		} else if (item_id == L1ItemId.EARTH_ENCHANT_WEAPON_SCROLL) { // 땅의 무기 강화 주문서
