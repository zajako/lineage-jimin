...
import bone.server.server.model.Instance.L1AuctionBoardInstance;
...
import bone.server.server.model.Instance.L1DwarfInstance;
import bone.server.server.model.Instance.L1EffectInstance;
import bone.server.server.model.Instance.L1FieldObjectInstance;
import bone.server.server.model.Instance.L1FurnitureInstance;
import bone.server.server.model.Instance.L1GuardInstance;
...
import bone.server.server.model.Instance.L1TeleporterInstance;
...
import bone.server.server.serverpackets.S_RangeSkill;
...
public class L1SkillUse {
	...
	private void sendIcon(L1PcInstance pc) {
		...
		case BLOOD_LUST: pc.sendPackets(new S_SkillBrave(pc.getId(), 6, _getBuffIconDuration)); Broadcaster.broadcastPacket(pc, new S_SkillBrave(pc.getId(), 6, 0)); break;
	...
	private void setDelay() {
		if (_skill.getReuseDelay() > 0) {L1SkillDelay.onSkillUse(_user, _skill.getReuseDelay());}
	}
	...
	private void runSkill() {
		...
		try {
			...
			for (Iterator<TargetStatus> iter = _targetList.iterator(); iter.hasNext();) {
				...
				break;
				case TRIPLE_ARROW:{ // 무기만 맞으면 사용가능하도록 by 코드
				...
				break;
				case FOU_SLAYER:{ // 무기만 맞으면 사용가능하도록 by 코드
					int weaponType =  _player.getWeapon().getItem().getType1();

					if (weaponType != 4 || weaponType != 24 || weaponType != 50) return;
				...
				case UNCANNY_DODGE:{
					if (cha instanceof L1PcInstance) {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.sendPackets(new S_PacketBox(S_PacketBox.DODGE, -5));
					}
				}
				break;
				case ICE_LANCE:
				...
				case SHOCK_STUN:{
					...
					if(diffLevel < -2){
						int[] stunTimeArray = { 700, 800, 900, 1000, 1100, 1200, 1500, 1700, 2000 };
						_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
					} else if(diffLevel >= -2 && diffLevel <= 3) {
						int[] stunTimeArray = { 800, 1000, 1400, 1500, 1600, 1700, 2000, 2200, 2300, 2400, 2500, 3000 };
						_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
					} else if(diffLevel >= 4 && diffLevel <= 10) {
						int[] stunTimeArray = { 1500, 1800, 2000, 2500, 2800, 2900, 3000, 3500, 4000 };
						_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
					} else if(diffLevel > 10) {
						int[] stunTimeArray = { 2000, 2500, 3000, 3400, 3500, 3600, 3700, 3800, 3900, 4000 };
						_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
					}
				...
				case CANCELLATION:{
					...
					if (cha instanceof L1PcInstance) {
						...
						if (pc.isPrivateShop()) {
							...
							Broadcaster.broadcastPacket(pc, new S_DoActionShop(pc.getId(), ActionCodes.ACTION_Shop, pc.getShopChat()));
						}
					}
				}
				break;
