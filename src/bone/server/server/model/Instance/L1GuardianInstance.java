...
public class L1GuardianInstance extends L1NpcInstance {
	...
	public L1GuardianInstance(L1Npc template) {
		super(template);
		synchronized (this) {
			if (getNpcTemplate().get_npcId() == 70848 || getNpcTemplate().get_npcId() == 70850) { // 엔트
	...
	public void searchTarget() {
		...
		for (L1PcInstance pc : L1World.getInstance().getVisiblePlayer(this)) {
			...
			if (!pc.isInvisble() || getNpcTemplate().is_agrocoi()) { // 인비지체크
				if (!pc.isElf()) { // 요정이아니면
					targetPlayer = pc;
					Broadcaster.wideBroadcastPacket(this, new S_NpcChatPacket(this, "$804", 1)); // 그대여. 목숨이 아까우면 빨리 이곳을 떠날지어다. 이곳은 그대같은 자가 더럽히지 못할 신성한 곳이다.
					break;
				}
			}
		}
