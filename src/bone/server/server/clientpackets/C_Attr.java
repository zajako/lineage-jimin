...
import bone.server.server.model.L1Question;
...
import bone.server.server.serverpackets.S_Resurrection;
import bone.server.server.serverpackets.S_ServerMessage;
import bone.server.server.serverpackets.S_SkillSound;
import bone.server.server.serverpackets.S_Trade;
import bone.server.server.templates.L1Alliance;
...
public class C_Attr extends ClientBasePacket {
	...
	public C_Attr(byte abyte0[], LineageClient clientthread) throws Exception {
		...
		case 622: // 설문
			c = readC();			
			if (c == 0)			L1Question.bad += 1;
			else if (c == 1)	L1Question.good += 1;			
			break;
		...
		case 1268:	// 데스매치에 입장하시겠습니까? (Y/N)
			c = readC();
			if(c == 0)					DeathMatch.getInstance().giveBackAdena(pc);
