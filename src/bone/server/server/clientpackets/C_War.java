...
public class C_War extends ClientBasePacket {
	...
	public C_War(byte abyte0[], LineageClient clientthread) throws Exception {
		...
		/*
		if (clan.getAlliance().CheckAlliance(enemyClan)) {
			player.sendPackets(new S_ServerMessage(ServerMessage.CANNOT_WAR_FROM_ALLIANCECLAN));
			return;
		}*/
		...
		if (enemyClan.getCastleId() != 0 && // 상대 크란이 성주로, 자캐릭터가 Lv25 미만
			...
			player.sendPackets(new S_SystemMessage("공성전을 선언하려면  레벨 61에 이르지 않으면 안됩니다.")); // 공성전을 선언하려면  레벨 25에 이르지 않으면 안됩니다.
			return;
		}
